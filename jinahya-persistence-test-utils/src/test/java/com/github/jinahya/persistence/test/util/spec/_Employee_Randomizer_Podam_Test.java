...
class _Employee_Randomizer_Podam_Test {

    @DisplayName("get() -> populated, with every excluded attribute left to its owner")
    @Test
    void get_ExcludedFieldsLeftAlone_() {
        _Employee_Randomized_Verifier.verifyInstantiated(new _Employee_Randomizer_Podam().get());
    }

    @DisplayName("get() -> a varying name on every call")
    @Test
    void get_Varies_() {
        _Employee_Randomized_Verifier.verifyVaries(new _Employee_Randomizer_Podam());
    }
}
