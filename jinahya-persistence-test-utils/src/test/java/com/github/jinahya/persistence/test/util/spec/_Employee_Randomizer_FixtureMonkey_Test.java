...
class _Employee_Randomizer_FixtureMonkey_Test {

    @DisplayName("get() -> populated, with every excluded attribute left to its owner")
    @Test
    void get_ExcludedFieldsLeftAlone_() {
        _Employee_Randomized_Verifier.verify(new _Employee_Randomizer_FixtureMonkey().get());
    }

    @DisplayName("get() -> a varying name on every call")
    @Test
    void get_Varies_() {
        _Employee_Randomized_Verifier.verifyVaries(new _Employee_Randomizer_FixtureMonkey());
    }
}
