...
class _Department_Randomizer_Podam_Test {

    @DisplayName("get() -> populated, with every excluded attribute left as the constructor left it")
    @Test
    void get_ExcludedFieldsLeftAlone_() {
        _Department_Randomized_Verifier.verify(new _Department_Randomizer_Podam().get());
    }

    @DisplayName("get() -> a varying name on every call")
    @Test
    void get_Varies_() {
        _Department_Randomized_Verifier.verifyVaries(new _Department_Randomizer_Podam());
    }
}
