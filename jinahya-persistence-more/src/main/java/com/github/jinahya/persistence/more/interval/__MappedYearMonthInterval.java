 ...
 * points from {@link __Interval}. {@link YearMonth#compareTo(YearMonth)} compares the year and then the month, which
 ...
 * classes further up, in {@link __MappedInterval}. Hibernate applies them. <strong>EclipseLink ignores them</strong>,
...
@AttributeOverride(
        name = "intervalStart",
        column = @Column(name = __MappedInterval.COLUMN_NAME_INTERVAL_START, nullable = true, insertable = true,
                         updatable = true, length = 7)
)
@AttributeOverride(
        name = "intervalEnd",
        column = @Column(name = __MappedInterval.COLUMN_NAME_INTERVAL_END, nullable = true, insertable = true,
                         updatable = true, length = 7)
)
@Convert(
...
public abstract class __MappedYearMonthInterval extends __MappedTemporalInterval<YearMonth>
        implements __DiscreteInterval<YearMonth> {
