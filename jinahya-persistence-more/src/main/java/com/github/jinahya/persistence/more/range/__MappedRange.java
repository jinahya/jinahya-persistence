 ...
 * public class DateRange extends __MappedRange<LocalDate> {
 ...
 * accessors here — {@link #isBounded()}, {@link __Range#isEmpty() isEmpty}, {@link #getLowerBoundType()} and the
...
public abstract class __MappedRange<C extends Comparable<? super C>> implements __Range<C> {
     ...
     *         ordered and every query really does use the index. But {@link __Range#contains(Comparable)} and
     *         {@link __Range#isEmpty()} answer by {@code compareTo}, so the database and the object would then
